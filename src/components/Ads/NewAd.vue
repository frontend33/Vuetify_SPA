  ...
  methods: {
    createAd () {
      if (this.$refs.form.validate() && this.imageSrc) {
        ...
        // Запускаем с dispatch новое событие которое хотим вызывать, вызываем создать объявление и передаем объект в него
        this.$store.dispatch('createAdActions', ad)
          .then(() => {
            // После передачи в actions, редиректим в мои объявление
